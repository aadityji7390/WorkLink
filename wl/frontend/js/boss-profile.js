document.addEventListener("DOMContentLoaded", async function () {
    ...
    function renderAvailability(available) {
        ...
        availabilityBtn.textContent = available ? "● Available" : "○ Unavailable";
        availabilityBtn.classList.toggle("is-unavailable", !available);
    }

    async function loadAvailability() {
        try {
            const r = await fetch("/api/boss/profile?phone=" + encodeURIComponent(viewedPhone));
            const d = await r.json();
            if (d.success) renderAvailability(d.available !== false);
        } catch (_) {}
    }

    if (availabilityBtn) {
        availabilityBtn.disabled = !isOwnProfile;
        availabilityBtn.title = isOwnProfile ? "Change your availability" : "Employer availability";
        availabilityBtn.addEventListener("click", async function () {
            if (!isOwnProfile) return;
            availabilityBtn.disabled = true;
            try {
                const current = !availabilityBtn.classList.contains("is-unavailable");
            ...
            finally { availabilityBtn.disabled = !isOwnProfile; }
        });
    }
    ...
    if (chatBtn && !isOwnProfile) {
        ...
        chatBtn.href = "chat.html?type=boss&phone=" + encodeURIComponent(viewedPhone) + "&name=" + encodeURIComponent((document.getElementById("headerName") || {}).textContent || "WorkLink User");
    }
